...
from analytics import TranspetroAnalytics
import pandas as pd
import json
...
try:
    ...
    # Isso é feito uma única vez para otimizar o tempo de resposta da API
    ...
    print(f"\n[FATAL ERROR] Falha ao inicializar a classe de análise. Verifique os arquivos: {e}")
    analytics = None
    # Caso a falha seja no carregamento, a API ainda pode subir, mas retornará 503.
...
def get_navegacao_diaria():
    ...
    # A coluna 'Data' precisa ser convertida para string
...
def get_risco_bioincrustacao_frota():
    """Métrica 8: Retorna o Risco de Bioincrustação (1-5) para toda a frota por mês."""
    check_analytics_ready()
    ...
    # A coluna 'Mês/Ano' precisa ser convertida para string
...
def get_clima_navio(ship_name):
    ...
    # Chama a função da API (que já faz a busca em chunks)
    ...
    # Converte a coluna de data/hora para string para JSON
...
if __name__ == '__main__':
    # Rode a aplicação Flask usando Uvicorn (melhor desempenho)
    print("\n--- INICIANDO SERVIDOR FLASK/UVICORN ---")
    print("Acesse: http://127.0.0.1:8000/")

    # Use o servidor Flask embutido para ambiente de desenvolvimento
    app.run(host="0.0.0.0", port=8000, debug=True)

    # Para produção, você usaria o uvicorn:
    # import uvicorn
